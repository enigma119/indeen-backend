...
/**
 * Guard to verify that the current user can access (view) a session.
 *
 * Access is allowed if:
 * - User is the mentor of the session
 * - User is the mentee of the session
 * - User is the parent of the mentee
 * - User is an ADMIN
 */
@Injectable()
export class CanAccessSessionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const session = await this.prisma.session.findUnique({
      ...
      include: {
        ...
        menteeProfile: { select: { userId: true, parentUserId: true } },
      },
    });
    ...
    // Check if user is the parent of the mentee
    if (session.menteeProfile.parentUserId === user.id) {
      return true;
    }
